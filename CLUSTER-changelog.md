...
## Update — July 7, 2026 (seventh pass)
Two changes, both about making the starburst feel stable and controllable rather than reactive:

9. **Positions stop moving on their own.** Every bucket action (sorting a card in, creating a bucket, renaming) used to trigger a full re-render that re-randomized every remaining card's spot — disorienting, since cards you weren't touching would jump. Fixed properly this time: once a card is shown anywhere in a CLUSTER session — whether that was its original random placement or somewhere a traveler dragged it — that position is remembered and reused on every subsequent render. Only a card CLUSTER has genuinely never shown before (e.g. a new idea captured while CLUSTER is open) gets a fresh random spot. Nothing else moves unless a traveler moves it.
10. **Lasso-select and move as a group.** Click-and-drag on empty starburst space now draws a selection box; releasing selects every card it overlaps (shown with a blue ring). Dragging any one of the selected cards afterward moves the *whole* selected group together, keeping their relative arrangement — onto empty space to arrange them as a cluster before committing, onto a bucket to sort all of them in at once, or onto another card to stack all of them into one new named header in a single naming prompt. A plain click on empty space (no drag) clears the selection. Dragging a card that isn't part of a selection behaves exactly as before — single card, unaffected.
...
- Tapping CLUSTER opens a full-screen-capable view scoped to that one bucket:
  - **Center** — the bucket's own loose, uncategorized ideas, scattered on first appearance (random position, tilt, and size, no row or grid alignment) and then stable — nothing rearranges itself once placed. Lasso-select several by dragging across empty space, then move them as a group. Drag a card (or a selected group) onto empty space to arrange things for yourself; drag directly onto another card to stack-and-name a new cluster on the spot.
  - **Shelf** — the bucket's existing sub-headers, always alphabetical, plus a **+** to spawn a new one. Sits along the bottom normally; flips to a column on the left in full-screen (⛶) mode. Tap a bucket to peek inside; drag a loose idea (or a selected group) onto it to sort in; drag one bucket onto another to nest it.
...
13. Open CLUSTER on a bucket with several loose cards, note their positions, then create a new bucket via + → confirm none of the existing loose cards move. Sort one card into a bucket → confirm the *remaining* cards don't move either.
14. Click-drag across empty starburst space to lasso 2–3 cards → confirm they show a blue ring. Drag one of them → confirm all selected cards move together, keeping their spacing. Drop the group onto a bucket → confirm all of them sort in at once. Try again, this time dropping the group onto another card → confirm one naming prompt covers the whole group.
15. Click once on empty starburst space (no drag) → confirm any active selection clears.
