...
(function(){
  ...
  function injectSeaOfIdeasCluster(){
    ...
    if(!document.getElementById('sea-cluster-style')){
      ...
      style.textContent='#s-sea-of-ideas-cluster .bar-dream-pp{background:#1a3a5c!important;border-color:#14305a!important;border-top-color:#2a5080!important}#s-sea-of-ideas-cluster .bar-dream-pp .tb{background:#d6eaf8!important;border-color:#a9cce3!important;color:#1a3a5c}#s-sea-of-ideas-cluster .bar-dream-pp .tb:hover:not(.dim){background:#5b9bd5!important;border-color:#5b9bd5!important;color:#fff}'
        ...
        +'.sb-shape-card{background:#fff;border-radius:16px;padding:16px;width:min(300px,88%);min-height:500px;box-shadow:0 10px 24px rgba(0,0,0,0.3);display:flex;flex-direction:column;box-sizing:border-box}'
        ...
        +'.sb-body-box{flex:1;display:flex;align-items:center;justify-content:center;text-align:center;min-height:170px;border-radius:10px;background:#f7f9fb;padding:14px;box-sizing:border-box;margin-bottom:10px;overflow:hidden;position:relative}'
        ...
        +'.sb-blue-btn{box-sizing:border-box;background:#eaf3fb;color:#1a3a5c;border:1px solid #b0a898;border-radius:12px;padding:10px 12px;font-size:15px;cursor:pointer;box-shadow:0 0 0 6px rgba(90,90,90,0.16),0 4px 10px rgba(26,58,92,0.22);flex:1 1 auto;min-width:40px}'
        ...
        +'.sb-blue-btn-sm{box-sizing:border-box;background:#eaf3fb;color:#1a3a5c;border:1px solid #b0a898;border-radius:11px;padding:8px 11px;font-size:12px;cursor:pointer;box-shadow:0 0 0 5px rgba(90,90,90,0.14),0 3px 8px rgba(26,58,92,0.2);flex:1 1 auto}'
  ...
  function _sboardMakeTile(item, size, straight, groupParentId){
    ...
    try{
      ...
      function renderGroup(headerRow, depth){
        ...
        if(directItems.length || subs.length){
          ...
          directItems.forEach(function(item){ grid.appendChild(_sboardMakeTile(item, cellSize, straight, headerRow.id)); });
      ...
      if(_sboardCurrentTopicId && _sboardAllRowsById[_sboardCurrentTopicId]){
        ...
        if(directIdeas.length===0 && childHeaders.length===0){
          ...
          if(directIdeas.length){
            ...
            directIdeas.forEach(function(item){ directGrid.appendChild(_sboardMakeTile(item, cellSize, true, _sboardCurrentTopicId)); });
  ...
  function openSbDetail(item){
    ...
    ov.innerHTML='<div class="sc-overlay-card sb-shape-card" style="text-align:center">'
      ...
      + headerListHTML
      + bodyHTML
      + '<div id="sb-hearts-row" style="font-size:14px;min-height:18px;margin:6px 0">'+_sboardHeartsHTML(heartCount)+'</div>'
